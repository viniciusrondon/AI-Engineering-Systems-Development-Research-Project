#!/usr/bin/env python3
"""
Base Model Performance Evaluation Script

This script evaluates the performance of the base GPT-4.1-nano-2025-04-14 model
...
class BaseModelEvaluator:
    """
    Evaluator class for testing base GPT-4.1-nano model performance
    on software requirements classification.
    """
    
    def __init__(self, model_name: str = "gpt-4.1-nano-2025-04-14"):
        """
        Initialize the evaluator with the base model.
    ...
    def predict_requirement_type(self, requirement_text: str) -> str:
        """
        Get prediction from the base model for a given requirement.
    ...
    def evaluate_model(self, test_data: List[Dict]) -> Dict:
        ...
        """
        print(f"\n🔍 Evaluating base model: {self.model_name}")
        print("=" * 60)
    ...
    def generate_confusion_matrix(self, save_path: str = None):
        ...
        plt.title(f'Confusion Matrix - Base Model ({self.model_name})')
    ...
    def generate_performance_chart(self, save_path: str = None):
        ...
        # Accuracy bar chart
        ax1.bar(['Base Model'], [accuracy], color='skyblue', alpha=0.7)
        ax1.set_ylabel('Accuracy (%)')
        ax1.set_title(f'Base Model Accuracy: {accuracy:.2f}%')
        ax1.set_ylim(0, 100)
...
def main():
    """
    Main function to run the base model evaluation.
    """
    print("🚀 Starting Base Model Performance Evaluation")
    print("=" * 60)
    ...
    # Load test data
    test_data = evaluator.load_test_data('data/dataset-test.jsonl')
    ...
    evaluator.generate_confusion_matrix('image/confusion_matrix_base_model.png')
    evaluator.generate_performance_chart('image/performance_base_model.png')
    
    # Save detailed results
    evaluator.save_detailed_results('base_model_results.json')
    
    print("\n✅ Base model evaluation completed successfully!")
